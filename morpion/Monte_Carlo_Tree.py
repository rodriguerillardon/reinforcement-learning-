...
from morpion import Morpion_board as c_board
import copy
import torch
...
class UCT_node():
    ...
    def expand(self, child_priors):
        ...
        if action_idxs ==[]:
            self.is_expanded = False
        self.action_index = action_idxs
        c_p[[i for i in range(len(child_priors)) if i not in action_idxs]] = 0.000
        if self.parent.parent == None:
            c_p = self.add_dirichlet_noise(action_idxs, c_p)
        self.child_priors = c_p

    def decode_n_move_pieces(selfself, board, move):
...
def UCT_search(game_state, num_reads, net, temp):
    root = UCT_node(game_state, move=None, parent=DummyNode())
...
def run_MCTS(args, start_idx=0, iteration=0):
    net_to_play = "%s_iter%d.pth.tar" % (args.neural_net_game, iteration)
    net = ConnectNet()

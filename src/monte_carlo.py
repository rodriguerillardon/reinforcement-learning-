...
class UCTNode():
    ...
    def expand(self, child_priors):
        ...
        action_idxs = self.game.get_moves();
        c_p = child_priors
        if action_idxs == []:
            self.is_expanded = False
    ...
    def backup(self, value_estimate: float):
        ...
        while current.parent is not None:
            ...
            if current.game.turn:  # same as current.parent.game.player = 0
                current.total_value += (1 * value_estimate)  # value estimate +1 = O wins
            elif not current.game.turn:  # same as current.parent.game.player = 1
...
def UCT_search(game_state, num_reads, net, temp):
    ...
    for i in range(num_reads):
        ...
        if leaf.game.check_win() == True or leaf.game.get_moves() == []:  # if somebody won or draw
    ...
    for idxx in tqdm(range(start_idx, num_games + start_idx)):
        ...
        while checkmate == False and current_board.get_moves() != []:
            if move_count < 11:
                ...
                t = 0.1
            states.append(copy.deepcopy(current_board.squares))
            board_state = copy.deepcopy(ed.encode_board(current_board))
            root = UCT_search(current_board, 666, connectnet, t)
            policy = get_policy(root, t);
            print("[CPU: %d]: Game %d POLICY:\n " % (cpu, idxx), policy)
            current_board = do_decode_n_move_pieces(current_board, \
                                                    np.random.choice(np.array([0, 1, 2, 3, 4, 5]), \
                                                                     p=policy))  # decode move and move piece(s)
            dataset.append([board_state, policy])
            print("[Iteration: %d CPU: %d]: Game %d CURRENT BOARD:\n" % (iteration, cpu, idxx),
                  current_board.squares, current_board.turn);
            print(" ")
            if current_board.check_win() == True:  # if somebody won
                if current_board.turn:  # black wins
                    value = -1
                elif not current_board.turn:  # white wins
                    value = 1
                checkmate = True
            move_count += 1
        dataset_p = []
